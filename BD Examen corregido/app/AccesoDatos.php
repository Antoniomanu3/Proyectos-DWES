...
class AccesoDatos
{
    ...
    private function __construct()
    {
        ...
        $this->stmt_moduser   = $this->dbh->prepare("update Usuarios set nombre=:nombre, password=:password, bloqueo=:bloqueo, saldo=:saldo, comentario=:comentario where login=:login");
        $this->stmt_creauser  = $this->dbh->prepare("insert into Usuarios (nombre, login ,password, bloqueo, saldo, comentario) Values(?,?,?,?,?,?)");
    }
    ...
    public function modUsuario($user): bool
    {
        ...
        $this->stmt_moduser->bindValue(':bloqueo', $user->bloqueo);
        $this->stmt_moduser->bindValue(':saldo', $user->saldo);
    ...
    public function addUsuario($user): bool
    {
        ...
        $this->stmt_creauser->execute([$user->nombre, $user->login, $user->password, $user->bloqueo, $user->saldo, $user->comentario]);
    ...
    public function bloquearUsuario(array $seleccionados)
    {
        $stringSeleccionados = implode("','", $seleccionados);
        $stmt1 = "UPDATE Usuarios SET bloqueo = 1 WHERE login IN ('$stringSeleccionados')";
        $stmt2 = "UPDATE Usuarios SET bloqueo = 0 WHERE login NOT IN ('$stringSeleccionados')";
        $this->dbh->exec($stmt1);
        $this->dbh->exec($stmt2);
    }
    ...
    public function incrementarSaldo(array $seleccionados)
    {
        $stringSeleccionados = implode("','", $seleccionados);
        $stmt1 = "UPDATE Usuarios SET saldo = saldo * 1.10 WHERE login IN ('$stringSeleccionados')";
        $this->dbh->exec($stmt1);
    }
