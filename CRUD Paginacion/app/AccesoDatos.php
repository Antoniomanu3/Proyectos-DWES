...
class AccesoDatos {
   ...
   public function numClientes ():int {
    $result = $this->dbh->query("SELECT nombre FROM Usuarios");
    $num = $result->num_rows;  
    return $num;
  } 
    ...
    public function getUsuarios ($primero,$cuantos):array {
       ...
       // echo "<h1> $primero : $cuantos  </h1>";
        $stmt_usuarios  = $this->dbh->prepare("select * from Usuarios limit $primero,$cuantos");
    ...
    public function modUsuario($user):bool{
        ...
        $stmt_moduser   = $this->dbh->prepare("update Usuarios set id=?, first_name=?, email=? , gender=?,ip_address=?,telefono=? where id=?");
    ...
    public function addUsuario($user):bool{
        ...
        $stmt_creauser  = $this->dbh->prepare("insert into Usuarios (id,first_name,email,gender,ip_address,telefono) Values(?,?,?,?,?,?)");
    ...
    public function borrarUsuario(String $id):bool {
        $stmt_boruser   = $this->dbh->prepare("delete from Usuarios where id =?");
